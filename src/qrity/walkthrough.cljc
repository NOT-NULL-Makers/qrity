(ns qrity.walkthrough
  "The fixed Version 1-M Numeric walkthrough of every Clause 7.1 stage.

  A staged pipeline that preserves each intermediate value for study, with
  the contracts in `qrity.spec` enforced between stages. Deliberately
  separate from `qrity.encode`: the generalized entry points share none of
  this machinery, and consumers that never run the walkthrough — the
  demonstration site among them — no longer carry its contract surface
  (measured at 50-70 KB of advanced-compiled ClojureScript)."
