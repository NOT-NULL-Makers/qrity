...
|---|---|---|---|
| 1. Data analysis | Clauses 7.2–7.4; Tables 2, 3, 5–7; Annex J | Minimum-single-mode classification and explicit Numeric, Alphanumeric, and Byte orchestration validate their exact payload contracts; Table 7 supports count-based Version 1–40 selection | Optimal segmentation and non-default ECI remain unimplemented |
| 2. Data encoding | Clauses 7.4.1, 7.4.3–7.4.5, 7.4.9, and 7.4.10; Tables 2, 3, 5–7; Annex I.2 | Selected-profile Numeric, Alphanumeric, and Byte construction; Byte uses mode `0100`, 8/16/16-bit octet counts, identity 8-bit packing, the common terminator/alignment, and alternating pads | Independent references cover every mode maximum and maximum-plus-one across all 160 profiles; Byte covers every octet and default ISO/IEC 8859-1 boundaries |
...
| 6. Data masking | Clauses 7.8.1–7.8.3; Tables 10–11 | Generalized explicit reversible transforms for all references 0–7; provenance-bound complete candidates with matching metadata; N1–N4 component scoring; all-minimum reporting and deterministic selection; fixed pipeline remains pinned to `010` | Independent mask formulas and complete-symbol scorers, literal N1–N4 boundaries, exact Version 1–40 × level × mask candidate construction, Annex I.2 winner, and fixed mask-2 regression |
...
| Data codewords and Numeric, Alphanumeric, and Byte capacities for all 160 Version/L-M-Q-H rows | Table 7, printed pp. 33–36 (PDF pp. 41–44) | All 320 added Alphanumeric/Byte cells were reconciled from the clean layout extraction; every printed maximum and maximum-plus-one is independently checked with the applicable Table 3 count width and Clause 7.4 packing formula | Canonical level facts, mode-capacity lookup, count-based version selection, and selected-profile packing for all three modes |
| Alphanumeric repertoire, values, count fields, and payload packing | Clauses 7.3.4 and 7.4.4; Tables 2, 3, and 5; printed pp. 21, 23, and 26–27 (PDF pp. 29, 31, and 34–35) | Rendered clean-PDF inspection; all 45 values and 2,025 ordered pairs; exact printed `AC-42` example; independent 9/11/13-bit count and padding reference across all 160 profiles | Canonical ordered repertoire shared with payload classification; raw payload bits and complete selected-profile Alphanumeric data codewords |
| Byte mode, default interpretation, count fields, and identity packing | Clause 6.1; Clauses 7.3.2, 7.3.5, and 7.4.5; Tables 2, 3, and 6; printed pp. 4, 20–21, 23, and 27–28 (PDF pp. 12, 28–29, 31, and 35–36) | Clean source inspection; all octets `0..255`; ISO/IEC 8859-1 boundary and Unicode-rejection checks; independent 8/16/16-bit count/padding reference across all 160 profiles | Canonical octet vectors, default-ECI text conversion without an ECI header, raw payload bits, and complete selected-profile Byte data codewords |
...
| Numeric, Alphanumeric, and Byte count bands, terminator, alignment, and padding | Tables 2–3; Clauses 7.4.3–7.4.5, 7.4.9–7.4.10 | Independent references across all 160 maximum capacities per mode; pinned V9→V10 and V26→V27 transitions; Numeric/Alphanumeric cover shortened terminators, while Byte's bit congruence always leaves the full four-bit terminator | Provisional selected-profile data-codeword construction for all three modes |
...
| Ordinary explicit data masks | Clauses 7.8.1–7.8.2; Table 10 | Independent Table 10 predicates, literal discriminating anchors, all 40 × 8 complete transforms, exact change sets, involution, function/metadata preservation, and remainder-module checks | Reversible low-level transform only; output structure does not prove mask provenance, scoring, or selection |
| Ordinary mask scoring and selection | Clause 7.8.3.1; Table 11; Figure 23; Annex I.2 | Independent component scorers; N1 maximal-run and N2 overlapping-block anchors; explicit N3 edge/both-side interpretation; integer N4 threshold neighbors; all 1,280 profile candidates; Annex I.2 mask-2 winner | Candidates bind final-message version/level, unmasked placement, mask transform, matching metadata, complete binary matrix, and component scores. ISO gives no tie-break; QRity returns all minima and provisionally selects the lowest numeric reference for reproducibility |

The selected-profile primitives construct and score all eight complete candidates and
select a deterministic minimum. Provisional mode-specific encoders compose them
after smallest-version selection across all
ordinary versions and correction levels. The
fixed Version 1-M/mask-2 stage walkthrough remains intact. Permanent JVM, Node, and
Babashka fixtures cover Versions 1, 2, 7, and 10; every runtime triple must be
byte-identical and decode exactly through both ZBar and OpenCV. The Alphanumeric
corpus covers all four levels, all Table 5 values, odd/even tails, Version-information
onset, and the Version 9→10 count-width transition: six runtime triples were
byte-identical and all 36 decoder checks recovered exact payloads. Shared
orchestration tests cross the Version 26→27 and high-version capacity boundaries.
The Byte corpus covers ASCII URL data, all four levels, Versions 1, 2, 3, 7, and
10, and its Version 9→10 count-width transition. Stable automatic API design and
broader high-density decoder evidence remain deferred. Numeric, Alphanumeric, and
Byte are complete single-segment encoding paths; default ISO/IEC 8859-1 Byte text
is supported without an emitted ECI header.
