...
|---|---|---|---|
| 1. Data analysis | Clauses 7.2–7.4; Tables 2, 3, 5, and 7; Annex J | Minimum-single-mode payload classification is implemented; Numeric orchestration validates ASCII digits; Table 7 Numeric, Alphanumeric, and Byte capacities support count-based Version 1–40 selection for explicit L/M/Q/H; direct Alphanumeric packing validates the exact Table 5 repertoire | Alphanumeric orchestration, Byte packing, optimal segmentation, and non-default ECI remain unimplemented |
| 2. Data encoding | Clauses 7.4.1, 7.4.3–7.4.4, 7.4.9, and 7.4.10; Tables 2, 3, 5, and 7; Annex I.2 | Fixed pipeline plus provisional selected-profile Numeric construction; isolated Alphanumeric data packing maps Table 5 values, encodes pairs in 11 bits and the final singleton in 6 bits, without mode/count fields or padding | Numeric reference covers all 160 profiles; Alphanumeric covers all 45 singletons, all 2,025 pairs, the printed `AC-42` vector, and generated concatenation/length properties |
...
| Data codewords and Numeric, Alphanumeric, and Byte capacities for all 160 Version/L-M-Q-H rows | Table 7, printed pp. 33–36 (PDF pp. 41–44) | All 320 added Alphanumeric/Byte cells were reconciled from the clean layout extraction; every printed maximum and maximum-plus-one is independently checked with the applicable Table 3 count width and Clause 7.4 packing formula | Canonical level facts, mode-capacity lookup, and count-based version selection; Numeric has selected-profile packing, Alphanumeric currently has payload-only packing |
| Alphanumeric repertoire, values, and payload packing | Clauses 7.3.4 and 7.4.4; Table 5; printed pp. 21 and 26–27 (PDF pp. 29 and 34–35) | Rendered clean-PDF inspection; all 45 values and 2,025 ordered pairs; exact printed `AC-42` example; independent generated reference and length formula | Canonical ordered repertoire shared with payload classification; `alphanumeric-data-bits` returns payload bits only |
...
select a deterministic minimum. Provisional `encode-numeric` composes them after
smallest-version selection across all ordinary versions and correction levels. The
