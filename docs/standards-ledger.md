...
| 6. Data masking | Clauses 7.8.1–7.8.2; Table 10 | Generalized explicit reversible transforms for all references 0–7 over canonical placed matrices; encoding modules including remainder bits are eligible, while function and unresolved metadata cells are preserved; fixed pipeline remains pinned to `010` | Independent formulas, literal row/column/product/division anchors, exact Version 1–40 × mask confinement, involution, V2 remainder fixture, and fixed mask-2 regression; scoring/selection remain deferred |
...
The low-level selected-profile primitives can now be composed manually through an
explicit mask and matching metadata into a complete construction matrix. The stable
complete generation pipeline remains Version 1-M Numeric/mask 2 until candidate
binding, scoring/selection, generalized orchestration, and decoder verification are
completed.
